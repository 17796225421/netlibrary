...
void EventLoop::handleRead()
{
    ...
    if (n != sizeof(one))
    {
        LOG_ERROR("EventLoop::handlerRead() reads %zd bytes instead of 8", n);
    }
}
...
void EventLoop::loop()
{
    ...
    while (!quit_)
    {
         ...
         * IO线程mainloop事先注册一个回调cb(需要subloop来执行)
         * wakeup subloop后，执行下面方法，执行之前mainloop注册的cb操作
        */
        doPendingFunctors();
    }
...
void EventLoop::quit()
{
    ...
    // 如果在其他线程中，调用的quit  在一个subloop(worker)中调用了mainloop(IO)的quit
    if (!isInLoopThread())
    {
        wakeup();
    }
